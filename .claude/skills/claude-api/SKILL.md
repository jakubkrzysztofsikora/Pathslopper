---
name: claude-api
description: Build LLM-powered applications with Claude. Use when integrating the Anthropic API or Claude SDK into an app — covers prompt engineering, tool use, streaming, agents, prompt caching, RAG patterns, and choosing between the Claude API and the Agent SDK. Triggers on imports of `anthropic`/`@anthropic-ai/sdk`, references to `/v1/messages`, or requests to add Claude features.
---

...
# srd-embeddings.json is generated by `npm run compute-srd-embeddings` and is
# NOT committed to git (33MB, in .gitignore). If the file exists at build time
# (CI generates it or developer ran the script), copy it. Otherwise create an
# empty stub so the app boots with zero-vector fallback retrieval.
COPY --from=builder --chown=nextjs:nodejs /app/src/lib/rag/srd-chunks.json ./src/lib/rag/srd-chunks.json
RUN if [ -f /app/src/lib/rag/srd-embeddings.json ]; then \
      cp /app/src/lib/rag/srd-embeddings.json ./src/lib/rag/srd-embeddings.json; \
    else \
      echo '[]' > ./src/lib/rag/srd-embeddings.json; \
    fi

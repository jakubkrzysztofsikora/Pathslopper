...
export function ZoneGeneratorPanel({ className }: ZoneGeneratorPanelProps) {
  ...
  async function handleGenerate() {
    ...
    try {
      const dna = getSnapshot();
      const res = await fetch("/api/zones/generate", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ dna, seed: { biome, encounterIntent } }),
      });
    ...
    >
      ...
      <div className="flex flex-col gap-3">
        ...
        <button
          ...
          className="self-start rounded-md bg-amber-600 px-4 py-2 text-sm font-medium text-zinc-950 hover:bg-amber-500 disabled:cursor-not-allowed disabled:bg-zinc-700 disabled:text-zinc-400"
        >
        ...
        {state.status === "success" && (
          <div
            className="rounded-md border border-zinc-700 bg-zinc-950 p-4 flex flex-col gap-3"
          ...
          >
            ...
            <div>
              <h4 className="text-xs uppercase tracking-wide text-zinc-400 mb-1">
                Narration (Markdown)
              </h4>
              <pre className="text-xs text-zinc-300 whitespace-pre-wrap border border-zinc-800 rounded p-2 max-h-48 overflow-auto">
                {state.markdown}
              </pre>
            </div>
